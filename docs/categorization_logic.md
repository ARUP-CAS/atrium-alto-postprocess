...
> decipher.
>
> The first now has a route that can finish the job: **`rule_domain_notation_categ`** (gate 0b in
> the table below, issue #30 D43). It is driven by **`DOMAIN_NOTATION_CATEG`**, which holds a
> **category name**, not an on/off switch, and **ships empty = off**, because @DanaKriv is the
> other half of the decision. Once set, *every* line `_RE_NOTATION_URL` recognises as a web or
> e-mail address gets that category, and the route runs **first** in the cascade, above even
> `rule_hard_sweep`: every signal below it measures how word-like a string is, and an address is
> not trying to be a word (`weird_ratio` is 1.00 on a URL, `detect_fused_words` fires on the
> address shape, an `@` halves `valid_word_ratio`). Its measured limit: nothing in it separates a
> correctly scanned address from a mis-scanned one, so the configured category applies to both —
> and `e-mail: officeauappmost.cz`, whose `@` the scanner lost, already reads `Clear` without the
> route. An unrecognised value fails at import rather than silently doing nothing.
>
> *(Corrected 2026-09-22. An earlier revision of this note described a `rule_domain_notation_clear`
> in gate 7 behind `SHORT_LINE_URL_CLEAR_ENABLE`. Neither exists in the code; that was an earlier
> design, replaced by the category-name route above before it landed on `master`.)*
>
> The second is not changed and is not described here as a defect: the short-garbage route's
> target category is a decision with an owner. ⏳ It has now been put to its owners — in the
> issue #30 thread after [comment 61](https://github.com/ufal/atrium-alto-postprocess/issues/30#issuecomment-5783543756)
> — and is open until they answer.
...
> [!NOTE]
> **(#30) The archive's own allow list, `setup/word_lists.txt` `[allowed]`.** A listed token is skipped
...
>
> How far that reaches is narrower than "a listed word is never damage". Because a line made only of
> listed words has no gibberish and no weirdness, `rule_short_garbage` (gate 5) cannot open on it. But
> the shape witness (5c) does not consult the list: in a line that mixes a listed word with other
> text, the witness — when armed — can still convict the line on the listed word's own shape (probed
> 2026-09-22 through `classify_TEXT.score_line()` with `ces_Latn` at 0.40 and perplexity 900: with
> `ssuti` listed and the witness on, `ssuti` alone is `Clear` and `ssuti vfetennl` is `Trash`, on
> `ssuti`'s `initial_geminate` clause alone — `vfetennl` satisfies no clause). Whether a listed
> word should also be exempt from the witness is ⏳ open for @DanaKriv and @david-spacil, and was put to them in the issue #30
> thread after comment 61.
...
| 0b | `rule_domain_notation_categ`                                              | **Off by default** (`DOMAIN_NOTATION_CATEG` empty). When it names a category, any line `_RE_NOTATION_URL` recognises as a web or e-mail address — short or long, correctly scanned or not                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | the category `DOMAIN_NOTATION_CATEG` names (`Clear` / `Noisy` / `Trash` / `Non-text` / `Empty`) | **Issue #30 D43.** Runs **first**, above `rule_hard_sweep`, because every signal below it measures how word-like a string is and an address is not trying to be a word. A category *name* rather than a switch, because this line has had three answers in the issue (`Trash`, then `Noisy` via D33, then `Clear` under @david-spacil's legibility definition, pending @DanaKriv). Reported under the matching threshold reason (`clear_threshold`, `noisy_threshold`, `trash_threshold`) or the pre-filter's (`non_text`, `empty`). Listed in `CONFIG_GATED_RULES`, so the rule-coverage sweep reports it `INERT`, not `DEAD`, while empty. Known limit: a mis-scanned address gets the same category as a correct one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 5b | `rule_domain_notation`                                                    | `is_domain_notation()` — archaeological / administrative **notation** shapes: grid and context refs (`II/C`, `I-VIII-c`, `KK-XIII`), labelled refs (`Lokalisace: MM-III`), counts (`1 ks`), abbreviation chains (`Reg.Bez.Aussig.`, `radius prox.sin.`), and — since D33 (`v1.5.1-beta`) — web and e-mail addresses anywhere in the line (`http://…`, `www.…`, an `@`-address, or an `e-mail:` label whose `@` the scan may have lost), via `_RE_NOTATION_URL`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | *(suppresses gate 5; no category of its own)*                                                   | Consulted at **two** narrow sites — the outer guard of `rule_short_garbage`, and the two section-1 routes that convict on perplexity alone (`rule_extreme_ppl`, `rule_absolute_ppl`). It confers no other exemption, unlike `is_structured_line()`, which is read at eleven places and vetoes `_has_strong_garbage_evidence()`. Recognises **notation, not vocabulary**: `malakofauna` and `Equus caballus` need a lexicon and are deliberately left to gate 5. Dimensions (`12,5 cm`) already satisfy `is_structured_line()` and are not duplicated here. **Not exempt from `rule_hard_sweep`** — see the note below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 5c | `_has_shape_garbage_evidence()` *(fires as `rule_short_garbage_witness`)* | **Off by default** (`SHORT_GARBAGE_WITNESS_ENABLE=false`). A phonotactic **shape** test over sub-tokens of ≥ `SHORT_GARBAGE_WITNESS_MIN_ALPHA` (def: 4) alphabetic characters: a **vowel run** (3 in a row by default, `SHORT_GARBAGE_WITNESS_VOWEL_RUN_MIN`; **4 when the raw FastText label is German or French**, `SHORT_GARBAGE_WITNESS_VOWEL_RUN_EXEMPT_LANGS` = `deu,fra` / `..._MIN_EXEMPT` = 4, issue #30 D44), the same character three times in a token of ≤ 8 letters, a doubled consonant in first position, or fewer than `SHORT_GARBAGE_WITNESS_VARIETY_MAX` (def: 0.50) distinct letters at ≥ 7 letters. Vetoed for the whole line by `has_cz_diacs()`, `is_structured_line()`, `is_domain_notation()` and `_RE_FUSED_GRID_REF` (a grid reference with its sub-letter fused, `S-VIIIb` — the hyphenless twin of a shape `is_domain_notation()` accepts); per sub-token it skips the `[neutral]` / `[short_exception]` / `[short_valid]` word lists, uppercase roman numerals (≤ 7 glyphs), Latin taxonomic suffixes (`-aceae`, `-idae`, …) and, once `SHORT_GARBAGE_LEXICON_PATH` points at a built table, any token attested in ≥ `SHORT_GARBAGE_LEXICON_MIN_DF` documents. It does **not** consult `setup/word_lists.txt` `[allowed]`. | *(no category of its own; wired as a second disjunct in gate 5, `070620f` + D15)*               | Reaches `oueussd`, `sektlll`, `cuxoaid`, `Tthts I`, `rragment`, `NINNNIC`, `vansasaasasa` — the garbage `_has_strong_garbage_evidence()` misses — while leaving `malakofauna`, `diapozitiv`, `Equus caballus`, `Skelettmaterial` and `vrstva` alone. It deliberately does **not** reuse `detect_fused_words()`'s other two clauses: `len(core) > 14` flags `Skelettmaterial`, and the 5+ consonant run flags `vrstva`, `ctvrtek`, `zmrzl` — ordinary diacritic-free Czech. The language label is the RAW `original_lang`, never the stored `lang`, which `remap_lang()` rewrites to Czech; a caller that passes no label gets the strict threshold. **Known limits:** the phonotactically legal residue (`edelite`, `vfetennl k.`) is out of reach by construction and needs a lexicon (#30 D14), with an optional `no_vocabulary` clause behind `SHORT_GARBAGE_LEXICON_CONVICT` (false); and the language label on a short damaged line is often wrong (#30 Z5). **Measured against the 2,064 scored rows of the gold sidecar** (`tools/gold/sidecars/issue30_gold_2067.csv`): with the document cascade it passes the adoption gate — errors 513 → 503, `Clear`-loss 38 → 38, 12 fixes / 2 breaks, p = 0.013, and 503 → 502 / 38 → 37 with the language split; per line (`--no-postprocessing`) it fails it, `Clear`-loss 53 → 54. @david-spacil's independent re-check of his 508 lines on the **default config (no lexicon)**, 2026-09-23: agreement 326 → 335/508 on `4ed309a`, 16 lines moved (11 fixed, 2 broken, 3 wrong either way). One of the two breaks was `S-VIIIb`, whose `_RE_FUSED_GRID_REF` guard had been deleted by an unrelated commit (`cc4990e`, 2026-09-18) and is restored and pinned by tests; the other, `Lokolieace: •VIII,` (gold `Noisy`), is damage a reader can decipher, which this route can only answer with `Trash`. **Confirmed by him on `3b02959`, with the guard restored: 336/508**, 15 lines moved (11 fixed, 1 broken, 3 wrong either way), `Lokolieace: •VIII,` the only break, and 0 of 508 changed with the flag off. It stays off pending @DanaKriv's annotation and her agreement on the `Trash` definition. |
