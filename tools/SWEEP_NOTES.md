...
> [!IMPORTANT]
> **"Final objective recalibration remains blocked until human-annotated gold labels exist" is no longer true, and was stale for a month.** The labels arrived on 2026-09-10 (`tools/gold/sidecars/issue30_gold_2067.csv`, 2,067 rows over 816 documents), and the rows they label are on the cluster in `/lnet/work/projects/atrium/alto_util/data_samples/DOC_LINE_CATEG` — verified 2026-09-17, 816/816 documents present. Pass `--gold-sidecar` plus `--gold-column gold_categ` and the objective stops being circular.
>
> Until you do, remember what the baseline means: the re-score reproduces the stored `categ` exactly, so `flip_rate == 0` is true **by construction** and every trial is scored on how little it changes the answer we already have. That makes the shipped config optimal by definition and scores a genuine improvement as damage. `tools/gold/GOLD.md` has the join contract and the standing caveat — 2,067 lines can tell an improvement from a regression; they cannot fit 37 constants.
